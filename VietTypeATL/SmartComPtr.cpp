...
INT_PTR SmartComPtrHelper::CompareUnknowns(const IUnknown *p1,
    ...
    IUnknown *u1 = NULL;
    if (ncp1 != NULL) {
        assert(SUCCEEDED(ncp1->QueryInterface(IID_IUnknown,
    ...
    IUnknown *u2 = NULL;
    if (ncp2 != NULL) {
        assert(SUCCEEDED(ncp2->QueryInterface(IID_IUnknown,
...
IUnknown *SmartComPtrHelper::Assign(IUnknown **dest, const VARIANT &src,
    ...
    // SmartComPtrHelper::Assign(): can't convert variant to unknown or dispatch!
    assert(0);
